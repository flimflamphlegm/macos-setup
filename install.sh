...
Installable components:
  ...
  DEFAULT (installed with no args or 'all'):
    ...
    skhd         global hotkey daemon (Ctrl+Shift+Esc dropdown) + skhdrc
    aerospace    AeroSpace tiling window manager + config
    tmux         tmux config (256-color fix) -> ~/.tmux.conf
    zsh          oh-my-zsh plugins (fzf, fzf-tab, zsh-autosuggestions,
...
install_tmux() {
  ...
  echo "==> Placing ~/.tmux.conf..."
  cp "tmux/tmux.conf" "$HOME/.tmux.conf"
}

# ---- Component: zsh (oh-my-zsh plugins + .zshrc) --------------------
install_zsh() {
  echo "==> Installing zsh plugins..."
  brew install fzf || true
  local ZC="${ZSH_CUSTOM:-$HOME/.oh-my-zsh/custom}"
  if [ ! -d "$HOME/.oh-my-zsh" ]; then
    echo "   !! oh-my-zsh not found. Install it first: https://ohmyz.sh" >&2
  fi
  mkdir -p "$ZC/plugins"
  # Clone external plugins (skip if already present).
  clone_plugin() {  # $1 = repo url, $2 = dir name
    if [ -d "$ZC/plugins/$2" ]; then echo "   $2 already present."
    else git clone --depth 1 "$1" "$ZC/plugins/$2" && echo "   cloned $2"; fi
  }
  ...
  echo "==> Placing custom zsh theme (engine2)..."
  mkdir -p "$ZC/themes"
  cp "zsh/themes/engine2.zsh-theme" "$ZC/themes/engine2.zsh-theme"
  echo "==> Placing ~/.zshrc..."
